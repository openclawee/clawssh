...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags="-s -w" -o /out/clawssh-wsproxy ./cmd/clawssh-wsproxy
...
COPY --from=build /out/clawssh-wsproxy /usr/local/bin/clawssh-wsproxy
